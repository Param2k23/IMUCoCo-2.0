...
Input  : (B, 9, T)    — 6D orientation (r6d) + 3-axis acceleration, channels-first
Output : (B, 24)      — raw logits (apply softmax externally / in loss)

Usage
-----
    from model import ResNet1D
    net = ResNet1D(n_classes=24, in_channels=9)
"""
...
class ConvBnRelu(nn.Sequential):
    ...
    def __init__(
        ...
        super().__init__(
            nn.Conv1d(in_c, out_c, kernel, stride=stride, padding=padding, bias=bias),
...
class ResBlock1D(nn.Module):
    ...
    def __init__(self, in_c: int, out_c: int, stride: int = 1):
        ...
        self.conv1 = ConvBnRelu(
            in_c, out_c, kernel=3, stride=stride, padding=1, bias=False
        )
        self.conv2 = nn.Sequential(
            nn.Conv1d(out_c, out_c, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm1d(out_c),
        )
...
class ResNet1D(nn.Module):
    ...
    def __init__(
        ...
        # Stem
        self.stem = ConvBnRelu(in_channels, base_filters, kernel=7, stride=1, padding=3)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x : (B, C, T)  where C=9 for vimu r6d+acc inputs
...
class CNN1D(nn.Module):
    ...
    def __init__(self, n_classes: int = 24, in_channels: int = 9, dropout: float = 0.3):
        super().__init__()
...
if __name__ == "__main__":
    ...
    for arch in ("resnet", "cnn"):
        ...
        x = torch.randn(8, 9, 120)
        out = net(x)
        n_params = sum(p.numel() for p in net.parameters() if p.requires_grad)
        print(
            f"{arch:8s}  in={tuple(x.shape)}  out={tuple(out.shape)}"
            f"  params={n_params:,}"
        )
